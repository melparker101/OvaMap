...
# This script is to be run from the project directory, e.g. PRJNA766716
# First argument should be raw_reads dir, second should be the output qc dir
...
if [[ ! -f index.txt ]]; then
  for f in *_R1_001.fastq.gz; do
    echo "${f%_R1_001.fastq.gz}" > index.txt
  done
fi
